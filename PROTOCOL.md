...
Installed environments and package caches, such as `.venv/`, `.julia_depot/`, `node_modules/`, conda environment directories, and tool caches, **SHOULD NOT** be committed. They **SHOULD** be listed in `.gitignore` and recreated from the dependency manifests, lockfiles, and commands committed to the repository. If a proprietary or manually installed tool such as MATLAB or Mathematica is required, `environment/README.md` **MUST** state the required version/toolboxes or paclets, what was tested, and whether an open alternative such as Octave was attempted.
