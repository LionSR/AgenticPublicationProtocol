...
├── reproduction/      optional: default output directory for figure/table reproduction scripts
...
The optional `reproduction/` directory is the default output location for artifacts regenerated by `code/figure-reproduction/` scripts (for example, `reproduction/figures/`). Keeping regenerated outputs here, rather than overwriting `paper/figures/`, lets a reader compare a fresh reproduction against the published artifact without losing the original. Publications **MAY** instead direct reproduction outputs to another location; in that case `code/figure-reproduction/README.md` **MUST** document the override on a per-script basis. The contents of `reproduction/` are not ground truth; the ground-truth artifacts live in `paper/`.
