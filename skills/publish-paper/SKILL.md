...
2. `/prepare-staging`
   - Asks whether the paper should cite the publication, using the repo name and version/tag recorded in the reproduction report; the author adds any link to the source manuscript before the paper is staged.
...
5. `/release-outcome`
   - Confirms the repo, tag, and any in-paper link settled earlier in the workflow; changing them at this point forces full re-validation.
