...
- **Environment Setup.** If the publication has runnable code, figure/table scripts, notebooks, or nontrivial build tools, `AGENTS.md` must include a concise "Environment Setup" section. It must match `README.md` and point to `environment/README.md` for details. Include the exact setup command(s) and the exact interpreter/runner prefix future commands should use, for example `.venv/bin/python`, `uv run`, `JULIA_DEPOT_PATH=.julia_depot julia --project=code`, `Rscript`, `octave`, `matlab -batch`, or `wolframscript`. State that local installed environment directories such as `.venv/`, `.julia_depot/`, and `node_modules/` are intentionally gitignored and recreated from the included manifests/lockfiles.
...
- Verify the Environment Setup section matches `README.md` and `environment/README.md`, and that runnable commands use the documented environment prefix.
...
Copy [`template/README.md`](../../template/README.md) to `publication-staging/README.md` and fill in each placeholder using information from phases 1-2 and the finalized `AGENTS.md`. The Setup section must contain the same concise environment setup information as `AGENTS.md`, including exact setup commands, environment runner prefixes, and a pointer to `environment/README.md` when present. The figure table should point to `code/figure-reproduction/README.md` and use the same status/script information as the `AGENTS.md` figure-reproduction summary, so the files stay checkable against each other.
