...
- **Level A — separate top-level agent (strongest).** A new top-level agent process with no shared in-memory context from this session: a fresh CLI invocation, a new chat in the same or a different agent, or a parallel agent run by another person. Working directory **MUST** be `publication-staging/`. This is the only level that fully attests to reader-agent behavior.
- **Level B — isolated in-session call (acceptable).** A single-purpose call made from inside this session — a subagent / Task tool invocation, or an equivalent isolated tool call — that receives only `publication-staging/` paths in its prompt and is instructed to answer purely from files it reads under that root. The publishing agent **MUST NOT** pre-summarize the answers or pass any context beyond the questions and the staging root; the isolated call must open the files itself.
